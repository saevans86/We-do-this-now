...
function getApi() {  
  ...
  }) // need display for current date, future dates with weather
  .then(function (data){    
    ...
    windEl.textContent = "Wind: " + data.list[0].wind.speed + " mph"; // re-add precip
    ...
    for (let i=1; i <= 4; i++) {
      ...
      forecastEl.textContent += " Wind: " + data.list[forecastIndex].wind.speed + " mph"; //re-add precip
